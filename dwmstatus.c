...
int get_mem(char* const dst, const size_t len) {
    int ret = ERROR;
    size_t cur_len = 0;
    char mem_total[BUF_SIZE] = {0}, mem_used[SMALL_BUF_SIZE] = {0},
         *temp = NULL;
...
static int reset_status(char* const dst,
                        const size_t len,
                        char* const net,
                        char* const bat,
                        char* const cpu,
...
static int create_status(char* const dst,
                         const size_t len,
...
int status_loop(void) {
    ...
    for (;; sleep(REFRESH_RATE)) {
        ...
        if (create_status(status, BUF_SIZE, net, bat, cpu, mem, vol,
                          cur_time)) {
            goto cleanup;
        }
